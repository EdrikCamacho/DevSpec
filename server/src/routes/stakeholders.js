...
router.post('/:projectId', requireAuth, async (req, res) => {
  const { name, role, influence, notes } = req.body;
  if (!name) return res.status(400).json({ error: 'Name is required' });
  const { data, error } = await supabaseAdmin.from(TABLE).insert({ project_id: req.params.projectId, name, role, influence: influence || 'media', notes }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'stakeholder_created', { name });
...
router.patch('/:projectId/:id', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'stakeholder_updated', req.body);
  res.json(data);
});
...
router.delete('/:projectId/:id', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'stakeholder_deleted', { id: req.params.id });
