...
router.post('/:projectId', requireAuth, async (req, res) => {
  ...
  const { data, error } = await supabaseAdmin.from(TABLE).insert({ project_id: req.params.projectId, name, type: type || 'humano', description }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'actor_created', { name });
...
router.patch('/:projectId/:id', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'actor_updated', req.body);
  res.json(data);
});
...
router.delete('/:projectId/:id', requireAuth, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  await logAudit(req.params.projectId, req.user.id, 'actor_deleted', { id: req.params.id });
